...
class InteractiveSession(object):
  ...
  def __init__(
    ...
    Args:
      ...
      metadata_connection_config: Optional metadata_store_pb2.ConnectionConfig
        ...
        pipeline_root directory will be used.
    """

    if not pipeline_name:
      pipeline_name = 'interactive-%s' % datetime.datetime.now().isoformat()
    if not pipeline_root:
      pipeline_root = tempfile.mkdtemp(prefix='tfx-')
      logging.warning(
          'InteractiveSession pipeline_root argument not provided: using '
    ...
    if not metadata_connection_config:
      ...
      metadata_sqlite_path = os.path.join(pipeline_root, 'metadata.sqlite')
  ...
  def run(self,
    ...
    driver_args = data_types.DriverArgs(
        ...
        interactive_artifact_resolution=True)
